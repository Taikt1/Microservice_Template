...
namespace Notification.App.Redis
{
    public class ResponseCache : IResponseCache
    {
        ...
        public async Task<TValue?> GetCacheAsJsonAsync<TValue>(string pattern, string key)
        {
            try
            {
                ...
                {
                    return default; // Hoặc bạn có thể throw new Exception nếu muốn cứng hơn
                }
            ...
            {
                return default; // hoặc throw lại lỗi nếu bạn muốn để bên ngoài xử lý
            }
        }
